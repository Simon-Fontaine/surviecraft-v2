...
module.exports = async (interaction, instance) => {
  ...
  if (customId !== "ticket-joueur-close-modal") return;

  const reason = interaction.fields.getTextInputValue("ticket-joueur-close-modal-reason");

  const transcriptChannel = guild.channels.cache.get(IDs.TICKET_JOUEUR_TRANSCRIPT);
  ...
  if (!ticket) {
    ...
    if (ticket.closed) {
      ...
      await ticketJoueurSchema.findOneAndUpdate({ channel_id: channel.id }, { closed: true });
      ...
      let file = [
        {
          ...
          name: `${ticket.ticket_id}-${ticket.owner_tag}-Transcript.txt`,
        },
      ];

      const embed = new EmbedBuilder()
        .setTitle(`Ticket #${ticket.ticket_id} de ${ticket.owner_tag}`)
        .setColor("#2f3136")
        .setDescription(
          [
